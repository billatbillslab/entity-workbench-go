...
`Store().Get` check alongside so the test says *"index empty"* and not *"tree lost it."*
**Mutation-checked before commit**: with the `Rebuild` call disabled it reports 0 matches.
The startup cost the repair buys is itself measured, not assumed —
`query_index_rebuild_bench_test.go` puts the size-dependent component of peer-open at
**~8.7 µs/entity, linear** (15.7 ms @ 100 · 23.1 ms @ 1k · 102.5 ms @ 10k), i.e. ~9 s at a
million entities. That number is the standing argument for backlog row **PR-1** (a SQLite-backed
query index against the store's own `*sql.DB`, which `SqliteStore.DB()` exists to permit): the
rebuild is the correct fix for *correctness* and a deferral on *shape*, and the two should not be
allowed to blur.

*Enforcement (AP40):* `shellcmd/cmd_compute_size_test.go` encodes each case through `put`'s own
`json.Unmarshal` → `ecf.Encode` path rather than building values in Go, so the bytes under test
are the bytes `put` stores. It carries a premise-guard that logs if a put-written size ever stops
round-tripping as `float64` — the way this class of test goes vacuous is the encoder moving under
it, silently making every case pass for the wrong reason.
